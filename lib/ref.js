...
export function assign({ deps, json }) {
  for (let loc of deps.overallOrder()) {
    let { obj, key } = getValue({ json, loc })
    if (mat.value({ json: obj, key })) {
      let refs = gather({ json: obj, key })
...
export function gather({ json, key }) {
  let regex = /(<<)?\s*(\$[\w\.\-\$]+)/g
  let match, refs = []
...
export function resolve({ json, loc, mixins={} }) {
  for (let key in json) {
    if (mat.value({ json, key })) {
      json[key] = gather({ json, key })
        .map((ref, i) => {
          let { name, op } = ref
          let [ firstKey ] = mat.firstKey({ name })
