...
export function mergeOp({ base, orig, loc, dest, source }) {
...
export function replaceOp({ base, loc, keyValue, replace, replaceWith }) {
  let { key, parent } = retrieveParentLoc({ base, loc })
  let value = parent[key]

  if (keyValue == "key") {
    replaceOpKey({ parent, key, replace, replaceWith, value })
  } else if (keyValue == "value") {
    replaceOpValue({ parent, key, replace, replaceWith, value })
  }
}

export function replaceOpKey({ parent, key, replace, replaceWith, value }) {
  delete parent[key]
...
export function replaceOpValue({ parent, key, replace, replaceWith, value }) {
  if (replace) {
